...
state_pv_df$state[is.na(state_pv_df$state)] <- "DC"
...
outcome_2020 <- read_csv("/Users/carinehajjar/Desktop/R studio/carine-h.github.io/data/popvote_bystate_1948-2020.csv") %>%
  filter(year == 2020) %>%
  mutate(state = state.abb[match(state,state.name)]) 
outcome_2020$state[is.na(outcome_2020$state)] <- "DC"
...
electoral <- electoral_votes %>%
  ...
  rename(state = X1) %>%
  mutate(state = state.abb[match(state,state.name)]) 
electoral$state[is.na(electoral$state)] <- "DC"
...
new_poll <- poll_2020_state %>%
  ...
  new_poll$state[is.na(new_poll$state)] <- "DC"
  
  new_data_poll <- new_poll %>%
    arrange(state)
...
# UPDATED 2020
hispanic_2020$State[hispanic_2020$State == "Washington DC"] <- "District of Columbia"
dat_2020 <- race_2020 %>%
  ...
  mutate(state = state.abb[match(state,state.name)]) %>%
  mutate(year = 2020)
dat_2020$state[is.na(dat_2020$state)] <- "DC"
...
state_econ$state[is.na(state_econ$state)] <- "DC"
...
e <- econ_update %>%
  ...
  filter(year == 2018)
e$state[is.na(e$state)] <- "DC"


g_2019 <- read_excel("/Users/carinehajjar/Desktop/R studio/carine-h.github.io/pred_data/state_2019_gdp.xlsx")
g_2019 <-  g_2019[-c(1:4),]
gdp_2019 <- g_2019 %>%
  mutate(state = state.abb[match(state,state.name)]) %>%
  mutate(year = 2019)
gdp_2019$state[is.na(gdp_2019$state)] <- "DC"

# new data: ASSUMING SAME GDP GROWTH ACROSS STATES AS IS NATIONALLY 
data_econ <- q2_2020 %>%
  mutate(year = 2020)
data_econ$state[is.na(data_econ$state)] <- "DC"
...
# 2020 PREDICTION
econ_pred <- predict(econ_fit, newdata = new_data_econ, na.rm = TRUE)
...
pred_electoral_count <- ensemble_tibble %>%
...
# electoral map table:
pred_update_map <- ensemble_tibble %>% 
 ...
 # intervals table
 pred_interval_table  <- as.data.frame(0.5*in_sample_poll + 0.25*in_sample_demo + 0.25*in_sample_econ) %>%
...
```{r}
# RMSE
as.data.frame(0.5*in_sample_poll + 0.25*in_sample_demo + 0.25*in_sample_econ) %>%
  mutate(state = new_data_poll$state) %>%
  select(state, lwr, fit, upr) %>%
...
# CLASSIFICATION ACCURACY 
    # states right/total
pred_winners <- as.data.frame(0.5*in_sample_poll + 0.25*in_sample_demo + 0.25*in_sample_econ) %>%
  mutate(state = new_data_poll$state) %>%
  select(state, lwr, fit, upr) %>%
  mutate(lwr = round(lwr,  digits = 2)/100, 
         fit = round(fit, digits = 2)/100, 
...
hist_comp <- hist(win_2020$fit*100 - win_2020$D_pv2p*100, 
     main = "Difference Between Democratic Vote Share \n Prediction and 2020 Outcome", 
     xlab = "(Corrected) Prediction - Actual Outcome", 
     ylim = c(0, 30), 
     xlim= c(-5, 15),
     col = "coral")
abline(v = mean( win_2020$fit*100 - win_2020$D_pv2p*100), col = "dodgerblue", lty = 2)
dev.off()
