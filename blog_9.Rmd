...
# CLASSIFICATION ACCURACY 
    # states right/total
pred_winners <- as.data.frame(0.5*in_sample_polls + 0.25*in_sample_demos + 0.25*in_sample_econs) %>%
  mutate(state = new_data_poll$state) %>%
  select(state, lwr, fit, upr) %>%
  mutate(lwr = round(lwr,  digits = 2)/100, 
         fit = round(fit, digits = 2)/100, 
...
win_2020 <- pred_winners %>%
  left_join(winners) %>%
  mutate(win = ifelse(winner == actual_winner, TRUE, FALSE))  # 45/50 were CORRECT
46/50
# 92% classification accuracy (iin terms of states won/lost)
```
...
# HISTOGRAM
hist(ensemble_tibble$pred - outcome_2020$D_pv2p*100, 
