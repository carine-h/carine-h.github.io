...
predict_intervals <- as.data.frame(0.5*in_sample_poll + 0.25*in_sample_demo + 0.25*in_sample_econ) %>%
...
predict_intervals2 <- as.data.frame(0.25*in_sample_poll + 0.25*in_sample_demo + 0.5*in_sample_econ) %>%
  mutate(state = new_data_poll$state) %>%
  select(state, lwr, fit, upr) %>%
  mutate(lwr = round(lwr,  digits = 2)/100, 
         fit = round(fit, digits = 2)/100, 
...
plot_ensemble <- plot_grid(plot_1, plot_2)

ggsave("~/Desktop/R studio/carine-h.github.io/images/ensemble_plot.png", plot_ensemble)
...
resid <- df_demo%>%
left_join(df_poll, by = c("state", "year")) %>%
...
1- (sum(resid$numerator, na.rm = TRUE)/sum(resid$denominator, na.rm = TRUE))
...
outsamp_dflist <- lapply(all_years, function(year){
  ...
  fit_out_poll <- lm(D_pv2p ~ avg_poll + as.factor(state), data = dat[dat$year != 2016,])
  fit_out_demo <- lm(D_pv2p ~ Black_change + Hispanic_change + Asian_change +
                         as.factor(state), data = dat_change[dat_change$year != 2016,])
  fit_out_econ <- lm(D_pv2p ~ gdp_growth*incumbent + as.factor(state), data = q3_econ[q3_econ$year != 2016,])

  mod_poll <- 0.5*predict(fit_out_poll, dat[dat$year == 2016,])
  mod_demog <- 0.25*predict(fit_out_demo, dat_change[dat_change$year == 2016,])
  mod_econ <- 0.25*predict(fit_out_econ, q3_econ[q3_econ$year == 2016,])

  cbind.data.frame(2016,
        poll_margin_error = mod_poll - true$D_pv2p,
        demo_margin_error = mod_demog - true$D_pv2p,
        econ_margin_error = mod_econ - true$D_pv2p,
        
        poll_winner_correct = (mod_poll > 50) == (true$D_pv2p > 50),
        demo_winner_correct = (mod_demog > 50) == (true$D_pv2p > 50),
        econ_winner_correct = (mod_econ > 50) == (true$D_pv2p > 50)
  )
})
