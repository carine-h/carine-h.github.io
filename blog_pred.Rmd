...
ensemble_tibble %>%
  ...
  gt() %>%
  tab_header(title = md("**2020 Electoral Vote Outcome Using Poll-Heavy Ensemble Prediction**"), 
               subtitle = "Biden Wins") %>%
...
ensemble_tibble2 %>%
  ...
  summarise(electoral_votes = sum(`2020`)) %>%
  gt() %>%
  tab_header(title = md("**2020 Electoral Vote Outcome Using Fundamentals-Heavy Ensemble Prediction**"), 
               subtitle = "Biden Wins") %>%
   cols_label(state_win = md("**Candidate**"),
               electoral_votes = md("**Total Electoral Votes**")) %>%
...
- do you take the average of each???
```{r}
in_sample_poll <- predict(fit_state_poll, newdata = dat, interval = "predict")


in_sample_demo <- predict(mod_demog_change, newdata = dat_change, interval = "predict")
in_sample_econ <- predict(econ_fit, newdata = q3_econ, interval = "predict")
```
...
data <- df_poll %>%
  left_join(df_demo, by = c("state", "year")) %>%
  left_join(df_econ, by = c("state", "year")) %>%
  left_join(pv, c("state", "year"))
