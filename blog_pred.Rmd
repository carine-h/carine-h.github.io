...
new <- rbind(tfc_df, new_data)

n <- new %>%
  filter(year == 2020)

predict(fit_tfc, newdata = new)
...
poll_pred_2020 <- predict(poll_fit, newdata = new_data_poll)
# if it's greater than 50, it's Biden, if it's less, it's Trump 
his <- tibble(state = new_data_poll$state, pred = poll_pred_2020)
...
# OUT OF SAMPLE CHECK
# plotting residuals --- how do I do this with this model ???
plot(dat_change$year, dat_change$D_pv2p, 
     main = "Demographic Model: \n True Democratic Popular Vote vs. Predicted Democratic Popular Vote", 
     xlab = "Year", 
     ylab = "Popular Vote Share",
     ylim = c(0, 100), 
     xlim = c(1950, 2020))
...
predict <- do(by_state, 
   as.data.frame(predict(mod_demog_change, dat_change))) %>%
  ungroup()

predict <- group_by(state) %>%
  summarise(pred = mean('predict(mod_demog_change, dat_change)'))

as.data.frame(predict(mod_demog_change, dat_change))
# predicted points
points(dat_change$year, predict(mod_demog_change, dat_change))
# legend formatting
legend("topright", 
       legend = c("Actual Outcomes", "Predicted Outcomes", "   Republican", "   Democrat"), 
       col = c("black", "black", "red", "blue"), 
...
demo_pred_2020 <- tibble(pred= predict(mod_demog_change, newdata = real_2020), state = real_2020$state)
