...
popvote_df <- read_csv("/Users/amandalitka/Desktop/carine R/popvote_1948-2016.csv")
...
# maybe correlate incumbency with attack and such or contrast - see if there's a relationship 
...
pv_tone%>%
  ...
  theme_classic() +
  labs(title = "Attack Ads Across Parties",
       x = "Year",
       y = "Percent",
       fill = "Incumbent") +
  facet_wrap("party")
...
pv_tone%>%
  ...
  geom_col(position = "dodge", width = 3) +
  theme_classic() +
  labs(title = "Contrast Ads Across Parties",
       x = "Year",
       y = "Percent",
       fill = "Incumbent") +
  facet_wrap("party")
...
pv_purp%>%
  ...
  geom_col(position = "dodge", width = 3) +
  theme_classic() +
  labs(title = "Policy Ads Across Parties",
       x = "Year",
       y = "Percent",
       fill = "Incumbent") +
  facet_wrap("party")
...
```
```{r}
# ad money spent and popular vote


# money spent by incumbent versus non-incumbent 
...
ad_campaigns %>%
  ...
  filter(year %in% c(2000, 2004, 2008, 2012), month > 7) %>%
  left_join(pop, by = c("year", "party")) %>%
  group_by(cycle, air_date, incumbent) %>%
  mutate(total_cost = sum(total_cost)) %>%
  ggplot(aes(x=air_date, y=total_cost, color= incumbent)) +
  # scale_x_date(date_labels = "%b, %Y") +
  scale_y_continuous(labels = dollar_format()) +
  scale_color_manual(values = c("blue","red"), name = "") +
  geom_line() + geom_point(size=0.5) +
...
# voter eligible and ad money spent 
